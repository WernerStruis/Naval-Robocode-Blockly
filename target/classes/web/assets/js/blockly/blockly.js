...
$('body').on('API', function () {
    ...
    Blockly.Toolbox.TreeNode.onClick_ = function(e){
        console.log("CLICK");
        e.preventDefault();
    };
    Blockly.workspace_.addChangeListener(inPlaceToolbox, this);
    ...
    $('.blocklyFlyout').before("<img src='assets/images/Thales_logo.png' id='flyoutLogo' style='width: 134px'>");
    $('.blocklyFlyout').after("<button class='btn btn-danger' onclick='returnToolbox()' id='returnFlyoutBtn' data-tag='return'>Return</button>");
...
function returnToolbox(){
    $('.blocklyFlyout').css('transform', 'translate(-' + $('.blocklyFlyout').width() + 'px, 0px)');
...
function removeItem(date) {
    var shipXMLList = JSON.parse(localStorage.getItem("SHIPXML"));
    console.log(shipXMLList);
    if (confirm($('[data-tag="confirm-delete"]').text() + date) == true) {
        if (shipXMLList) {
            shipXMLList.ship = shipXMLList.ship.filter(function (obj) {
                return obj.date !== date;
            });
            //var data = $.grep(shipXMLList, function(e){
            //    return e.date != date;
            //});
            //for (var i = 0; i < shipXMLList.ship.length; i++) {
            //    console.log(shipXMLList.ship[i].date);
            //    if (shipXMLList.ship[i].date == date) {
            //        console.log("BINGO");
            //        shipXMLList.ship.splice(i - 1,i);
            //        //delete shipXMLList.ship[i];
            //    }
    ...
    if (shipXMLList) {
        ...
        for (var i = 0; i < shipXMLList.ship.length; i++) {
            if (shipXMLList.ship[i].API == API) {
                var loadfn = 'loadShipXMLFromDate("' + shipXMLList.ship[i].date + '")';
                var delfn = 'removeItem("' + shipXMLList.ship[i].date + '")';
                $('#shipLoadList').append('<li class="list-group-item"><a onclick=\'' + loadfn + '\'>' + shipXMLList.ship[i].date + '</a><a onclick=\'' + delfn + '\'><span class="glyphicon glyphicon-remove"></span></a></li>');
                ++count;
            }
        }
        if (count > 0) {
...
function loadAfterInject() {
    setTimeout(function () {
        if (!Blockly.getMainWorkspace()) {
            ...
            console.log("Loading workspace from XML...");
